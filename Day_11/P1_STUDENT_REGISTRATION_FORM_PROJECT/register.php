<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $sql = "INSERT INTO st_detail(st_fname,
               ...
               st_address,st_city,st_pincode, activation_code) 
        ...
        $to = $_POST['st_email'];
        $subject = 'Account Activation For Student Registration Website';
        $output='<p>Dear user,</p>';
        $output.='<p>Please click on the following link to activate your account.</p>';
        $output.='<p>-------------------------------------------------------------</p>';
        $output.='<p><a href="http://localhost:8080/Internship/Day_11\P1_STUDENT_REGISTRATION_FORM_PROJECT/active.php?st_email='.$_POST['st_email'].'&uniqid='.$uniqid.'" target="_blank">Click Me!</a></p>'; 
        $output.='<p>-------------------------------------------------------------</p>';
        $output.='<p>Please be sure to copy the entire link into your browser.</p>';
        $output.='<p>However, you may want to log into your account and change your security password as someone may have guessed it.</p>'; 
        $output.='<p>Thanks,</p>';
        $output.='<p>Student Registration Website Team</p>';
        $message = $output;
        echo smtp_mailer($to,$subject,$message);
        if($query){
            header("Location: login.php?active_msg_req=success");
        }
    }    
?>
<?php 
